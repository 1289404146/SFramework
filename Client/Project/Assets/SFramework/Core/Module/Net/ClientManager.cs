...
public class ClientManager : BaseMono,IManager
{
    ...
    private void OnProcessProtolDataCallback(ActionCode actionCode, byte[] data)
    {
        msgs.Enqueue(new Msg() { code = actionCode, datas = data });
    }
    ...
    public void SendRequestByProtol(RequestCode requestCode, ActionCode actionCode, byte[] data)
    {
        byte[] bytes = Message.PackDataByte(requestCode, actionCode, data);
        clientSocket.Send(bytes);
    }
    public void SendMassage(IMessage message)
    {
