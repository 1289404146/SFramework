...
public class Message
{
    ...
    public void ReadMessage(int newDataAmount, Action<ActionCode, string> processDataCallback)
    {
        ...
        public void ReadMessageFromProtol(int newDataAmount, Action<ActionCode, byte[]> processDataCallback)
    {
        startIndex += newDataAmount;
        while (true)
        {
            ...
            if ((startIndex - 4) >= count)
            {
                ...
                ActionCode actionCode = (ActionCode)BitConverter.ToInt32(data, 4);
                //string s = Encoding.UTF8.GetString(data, 8, count - 4);
                byte[] b = data.Skip(8).Take(count - 4).ToArray();

                processDataCallback(actionCode, b);
                Array.Copy(data, count + 4, data, 0, startIndex - 4 - count);
                startIndex -= (count + 4);
            }
            else
