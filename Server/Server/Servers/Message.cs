...
class Message
{
    ...
    public void ReadMessageFromProtol(int newDataAmount, Action<RequestCode, ActionCode, byte[]> processDataCallback)
    {
        startIndex += newDataAmount;
        while (true)
        {
            ...
            if ((startIndex - 4) >= count)
            {
                ...
                RequestCode requestCode = (RequestCode)BitConverter.ToInt32(data, 4);
                ActionCode actionCode = (ActionCode)BitConverter.ToInt32(data, 8);
                byte[] b= data.Skip(12).Take(count - 8).ToArray();
                processDataCallback(requestCode, actionCode, b);
                Array.Copy(data, count + 4, data, 0, startIndex - 4 - count);
                startIndex -= (count + 4);
            }
            else
    ...
    public static byte[] PackDataByte(RequestCode requestData, ActionCode actionCode, byte[] data)
    {
